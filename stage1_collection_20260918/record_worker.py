...
from record_common import safe,save,receive,topics,validate_spec
...
def main():
    ...
    def qos(key):
        return QoSProfile(depth=2 if key.endswith('_image') else 128,reliability=ReliabilityPolicy.RELIABLE,
                          durability=DurabilityPolicy.TRANSIENT_LOCAL if key in ('static','session','waypoints') else DurabilityPolicy.VOLATILE)
    rclpy.init(args=[]);node=rclpy.create_node('stage1_record_'+role)
        ...
        if role=='publish':
            ...
            while True:
                ...
                if kind=='state':
                    ...
                    msg=PoseStamped();stamp(msg,ns,'world');p=h['gt_center']
                    msg.pose.position.x,msg.pose.position.y,msg.pose.position.z=p['position']
                    msg.pose.orientation.w,msg.pose.orientation.x,msg.pose.orientation.y,msg.pose.orientation.z=p['quaternion_wxyz'];publish('gt_pose',msg)
                    msg=Vector3Stamped();stamp(msg,ns,'world')
                    msg.vector.x,msg.vector.y,msg.vector.z=h['velocity_world_m_s'];publish('gt_velocity',msg)
                elif kind=='image':
                    ...
                    msg=PointCloud2();stamp(msg,h['native_header_ns'],'drone_1/lidar')
                    ...
                    msg.data=array.array('B',body);publish('lidar',msg)
                else:raise ValueError('Unexpected record '+str(kind))
                h['publisher_wall_ns']=str(time.time_ns());string('mapping',h)
                rclpy.spin_once(node,timeout_sec=0)
            report.update(status='complete');save(case/'publisher_done.json',report)
            ...
            def offered(k):
                ...
                return [Qos(QosHistory.KEEP_LAST,2 if k.endswith('_image') else 128,QosReliability.RELIABLE,
                            QosDurability.TRANSIENT_LOCAL if k in ('static','session','waypoints') else QosDurability.VOLATILE,
            ...
            def callback(k):
                def receive_raw(raw):
                    ...
                    receipts.write(json.dumps({'key':k,'sequence_on_topic':counts[k]-1,'wall_ns':str(wall),'mono_ns':str(mono),'bytes':len(raw)},separators=(',',':'))+'\n')
                return receive_raw
