...
spec={'schema':'stage1_recording_v1','mode':'smoke' if ticks<=60 else ('pilot' if ticks==300 else 'route'),
      ...
      'color_plane_enabled':False,'lidar_enabled':True,'epoch':int(rid[:8],16),'request_id':rid}
...
for name in ('progress.json','producer_report.json','driver_report.json','publisher_done.json','subscriber_done.json','publish_error.json','subscribe_error.json'):
    if (case/name).exists():
        ...
        result[name]=d if name not in ('producer_report.json','driver_report.json') else {k:d[k] for k in ('status','error','traceback','online_qr','state_count','image_count','lidar_count','resources') if k in d}
...
'''));samples.append(result)
    ...
    print(json.dumps({'progress':result.get('progress.json'),'driver':result.get('driver_report.json',{}).get('status'),
                      'producer':{k:v for k,v in result.get('producer_report.json',{}).items() if k in ('status','error','online_qr')},
