...
from mro_runtime.paths import (read_json, atomic_replace_json, ensure_private_file,
                               write_new_bytes)
from stage1_supervisor import load_plan, proc_stat, still_live
...
def main():
    ...
    if not args.execute:
        print('Read-only resource snapshot. Add --execute to start the separate stage1 GPU1 workbench.')
        return
    if PROJECT.is_symlink() or PROJECT.resolve(strict=True) != PROJECT:
        raise ValueError('Unexpected stage1 workspace resolution')
    lock_rel = 'stage1/runtime/run/workbench_start.lock'
    if not (ROOT / lock_rel).exists():
        try:
            write_new_bytes(ROOT, lock_rel, b'')
        except FileExistsError:
            pass
    encode_lock_rel = 'stage1/runtime/run/video_encode.lock'
    ...
    with ensure_private_file(ROOT, lock_rel).open('r+') as lock, \
        ...
        manifest = ROOT / 'stage1/manifests/current_view_supervisor.json'
        if manifest.exists():
        ...
        cfg = read_json(ROOT, 'stage1/config/viewing_session.json')
        launch_id = cfg.get('consumed_by_launch_id')
        if launch_id:
            owner = read_json(ROOT, 'stage1/runtime/run/' + launch_id + '/owner.json')
            ...
            if owner.get('remaining_owned_processes'):
                ...
                atomic_replace_json(ROOT, 'stage1/runtime/run/' + launch_id + '/cleanup_reverification.json',
        ...
        atomic_replace_json(ROOT, 'stage1/config/viewing_session.json', cfg)
        child = None
        try:
            load_plan()
            stamp = datetime.datetime.now(datetime.timezone.utc).strftime('%Y%m%dT%H%M%S%fZ')
            log_rel = 'stage1/runtime/logs/view_supervisor_' + stamp + '.log'
            ...
            with ensure_private_file(ROOT, log_rel).open('ab') as log:
                child = subprocess.Popen(['/usr/bin/python3', '-B',
                    str(PROJECT / 'scripts/stage1_supervisor.py'), '--execute'], cwd=ROOT,
            ...
            print('Starting: wait for stage_status=stage1_open before WebRTC Connect.')
        except Exception:
            ...
            if child is not None and child.poll() is None:
                ...
                try:
                    ...
                    latest = read_json(ROOT, 'stage1/config/viewing_session.json')
                    latest.update(enabled=False, stop_requested=True)
                    atomic_replace_json(ROOT, 'stage1/config/viewing_session.json', latest)
