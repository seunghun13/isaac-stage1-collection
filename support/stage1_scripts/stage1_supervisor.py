...
POLICY = 'stage1/config/viewing_session.json'
...
def load_plan():
    ...
    if PROJECT.is_symlink() or PROJECT.resolve(strict=True) != PROJECT:
        raise ValueError('Unexpected stage1 workspace resolution')
    cfg = read_json(ROOT, POLICY)
    gate = read_json(ROOT, 'stage1/config/launch_policy.json')
    ...
    expected = {'public_ip': '165.246.193.194', 'signaling_port': 49100,
    ...
    env = read_json(ROOT, 'stage1/config/runtime_environment.json')
    ...
    omni = ensure_private_file(ROOT, 'stage1/runtime/config/omniverse/omniverse.toml')
    ...
    ensure_private_file(ROOT, 'stage1/config/user.config.json')
    env.update(PATH='/usr/bin:/bin', LANG='C.UTF-8', LC_ALL='C.UTF-8',
               ...
               OPTIX_CACHE_PATH=str(confined_path(ROOT, 'stage1/runtime/cache/optix')),
    ...
    args = [str(kit), str(app), '--no-window', '--no-ros-env', '--portable-root',
            ...
            '--/app/livestream/publicEndpointAddress=165.246.193.194', '--/app/livestream/port=49100',
    ...
    # Original project/installation/assets stay read-only. The stage1
    ...
    directory = ensure_directory(ROOT, 'stage1')
...
def execute(plan):
    ...
    run = confined_path(ROOT, f'stage1/runtime/run/{launch_id}')
    ensure_directory(ROOT, run.parent)
    run.mkdir(mode=0o700)
    for path in ('stage1/runtime/logs', 'stage1/runtime/cache/optix', 'stage1/outputs/manual', 'stage1/outputs/replicator', 'stage1/outputs/streaming'):
        ensure_directory(ROOT, path)
    console = confined_path(ROOT, f'stage1/runtime/logs/{launch_id}.console.log')
    kitlog = confined_path(ROOT, f'stage1/runtime/logs/{launch_id}.kit.log')
